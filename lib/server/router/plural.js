      ...
      if (key !== 'callback' && key !== '_') {
        ...
        chain = chain.filter(element => {
          return arr.map(function (value) {
            ...
            if (isRange) {
              const isLowerThan = /_gte$/.test(key); // check is date

              const isDateValue = new Date(Date.parse(value));
              const dateElementValue = new Date(elementValue);

              if (isDateValue.toISOString() === value) {
                return isLowerThan ? isDateValue.getTime() <= dateElementValue.getTime() : isDateValue.getTime() >= dateElementValue.getTime();
              }
